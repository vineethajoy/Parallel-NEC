...
C     THEIR EMPLOYEES, NOR ANY OF THIR CONTRACTORS, SUBCONTRACTORS, OR
...
C
C     Changes made by J Bergervoet:
C         Status of output files set to 'UNKNOWN'
C         Changed the DB20 routine to avoid compiler warning
C         Changed the SECONDS routine (see below.)
C     Last changed: Jan 15, 1996
C
      PROGRAM NEC
      INCLUDE 'nec2dpar.inc'
      PARAMETER (IRESRV=MAXMAT**2, LOADMX=MAXSEG)
      IMPLICIT REAL*8(A-H,O-Z)
      CHARACTER AIN*2,ATST*2,INFILE*80,OUTFILE*80,INMSG*48,OUTMSG*40
      REAL*8 HPOL,PNET
      INTEGER*2 GPWNXY(2)
      LOGICAL*4 GetPut,LGTPT
C      COMPLEX*16  CM,FJ,VSANT,ETH,EPH,ZRATI,CUR,CURI,ZARRAY,ZRATI2
      COMPLEX*16 CM
      ALLOCATABLE CM
      ...
      INTEGER    IAM, NPROCS, ICTXT, MYROW, MYCOL,eerr,PRIMA
      INTEGER    NPROW, NPCOL,R,C,MM, Split2d,MA,NA,NNN
C      INTEGER IAM,NPROCS,RRR,CCC,MYROW,MYCOL,NPROW,NPCOL,ICTXT
      COMMON /PARA/ ICTXT,IAM,NPROCS,MYROW,MYCOL,NPROW,NPCOL,R,C,MM,NNN
...
c      COMMON /CMB/CM(2250000)
c      COMMON /CMB/CM(46656)
...
C***
     ...
     &SITEMP(MAXSEG),BITEMP(MAXSEG), CM(:)
     ...
     1 'NX','EN','TL','PT','KH','NH','PQ','EK','WG','CP','PL'/
      ...
      DATA NSMAX,NETMX/30,30/,NORMF/200/

      CALL BLACS_PINFO(IAM,NPROCS)

      NPROW = Split2D(sqrt(real(NPROCS)),NPROCS)
      NPCOL = NPROCS/NPROW
      R = 64
      C = 64
      PRIMA=0
...
C      ALLOCATE (CM(MM*MM))
      ...
      Write(*,*) 'Numerical Electromagnetics Code, ',
     &                            'double precision version (nec2d)'
      Write(*,*) 'developed at Lawrence Livermore Lab., ',
     & 'Livermore, CA., by G. Burke'
      Write(*,*)
     & '(burke@icdc.llnl.gov) and A. Poggio.'
      Write(*,*)
     & 'Fortran file was created 4/11/80, last changed: Jan 15, 96, by'
      Write(*,*)
     & 'J. Bergervoet (bergervo@prl.philips.nl)'
      Write(*,*)
      Write(*,*) 'Maximum number of segments in core : MAXMAT=',MAXMAT
      If(MaxSeg.ne.MaxMat) 
     &Write(*,*) 'Maximum when using swap files      : MAXSEG=',MAXSEG
      WRITE(*,*)
      END IF

      OPEN (UNIT=8,FILE='files.ini',STATUS='OLD')
C706   WRITE(*,700)
C700   FORMAT(' ENTER NAME OF INPUT FILE >',$)
CC      READ(*,'(A)',ERR=702) INFILE
      READ (8,'(A)') INFILE
CC      IF (IAM.EQ.0) THEN
C         INFILE = 'monop.nec'
C
      ...
      GOTO 707
702   WRITE(*,*) ' ERROR opening input file'
C      GOTO 706   
C707   WRITE(*,703)
C703   FORMAT(' ENTER NAME OF OUTPUT FILE >',$)
CC      READ(*,'(A)',ERR=704) OUTFILE
...
C         OUTFILE = 'monop.out'
      ...
      CLOSE (8)
      GOTO 701
704   WRITE(*,*) ' ERROR opening output file'
C      GOTO 707

C-- To use out-of-core factorization open swapfiles with include file
C-- by uncommenting the line below label 701.
C-- Maybe the swapfiles should be opened conditionally in routine FBLOCK
C-- Maybe the swapfiles could also be closed after routine ...

701   Continue
C      Include 'openswap.inc'
      GO TO 705
...
C
C      IF (IAM.EQ.0) THEN
         MA=MAX(1,NUMROC(NEQ,R,MYROW,0,NPROW))
         NA=MAX(1,NUMROC(NEQ,C,MYCOL,0,NPCOL))
         MM = MA
         NNN = NA
C         CALL IGEBS2D( ICTXT, 'All', ' ', 1, 1, MM, 1 )
C      ELSE
C         CALL IGEBR2D( ICTXT, 'All', ' ', 1, 1, MM, 1, 0, 0)
C      END IF
      IF (PRIMA.EQ.0) THEN
         WRITE(*,*)
         WRITE(*,*)MYROW,MYCOL,MA,NA,MM
	   ALLOCATE (CM(MA*NA))
         PRIMA=1
      END IF
C      ALLOCATE (CM(MM*MM))
...
C
     ...
     &D,DETER
      INTEGER ICTXT,NPROCS, NPROW, NPCOL, MYROW, MYCOL, II, JJ, FORME
      INTEGER R, C,MM, NNN
      COMMON /PARA/ ICTXT,IAM,NPROCS,MYROW,MYCOL,NPROW,NPCOL,R,C,MM,NNN
      COMMON /DATA/ X(MAXSEG),Y(MAXSEG),Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),
     &ALP(MAXSEG),BET(MAXSEG),WLAM,ICON1(2*MAXSEG),ICON2(2*MAXSEG),
...
C
      ...
      DO 3 I=1,JSNO
      JSS=JCO(I)
      IF (FORME(MYROW,MYCOL,NPROW,NPCOL,JSS,IPR).EQ.1) THEN
         CALL INDEXCHANGE(NPROW,NPCOL,JSS,IPR,II,JJ)
         CM(II,JJ)=CM(II,JJ)-(AX(I)+CX(I))*ZAJ
      END IF
 3	CONTINUE

C3     CM(JSS,IPR)=CM(JSS,IPR)-(AX(I)+CX(I))*ZAJ
...
9     DETER=DETER+D(KK)
      ...
      IF (FORME(MYROW,MYCOL,NPROW,NPCOL,J,I).EQ.1) THEN
         CALL INDEXCHANGE(NPROW,NPCOL,J,I,II,JJ)
         CM(II,JJ)=DETER
      END IF
...
10    DETER=DETER+D(KK)*SSX(K,KK)
      ...
      IF (FORME(MYROW,MYCOL,NPROW,NPCOL,KA,I).EQ.1) THEN
         CALL INDEXCHANGE(NPROW,NPCOL,KA,I,II,JJ)
         CM(II,JJ)=DETER
      END IF
...
C
      COMPLEX*16 CM,CW,ETK,ETS,ETC,EXK,EYK,EZK,EXS,EYS,EZS,EXC,EYC,EZC
      INTEGER IAM,NPROCS,ICTXT,NPROW,NPCOL,R,C,MYROW,MYCOL,FORME,MM
      COMMON /PARA/ ICTXT,IAM,NPROCS,MYROW,MYCOL,NPROW,NPCOL,R,C,MM,NNN
      COMMON /DATA/ X(MAXSEG),Y(MAXSEG),Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),
     &ALP(MAXSEG),BET(MAXSEG),WLAM,ICON1(2*MAXSEG),ICON2(2*MAXSEG),
     &ITAG(2*MAXSEG),ICONX(MAXSEG),LD,N1,N2,N,NP,M1,M2,M,MP,IPSYM
     ...
     1CON(10),NPCON
      COMMON /DATAJ/ S,B,XJ,YJ,ZJ,CABJ,SABJ,SALPJ,EXK,EYK,EZK,EXS,EYS,
     &EZS,EXC,EYC,EZC,RKH,IND1,INDD1,IND2,INDD2,IEXK,IPGND
      DIMENSION CM(MM,1), CW(MM,1), CAB(1), SAB(1)
...
C     NORMAL FILL
      DO 17 IJ=1,JSNO
      JX=JCO(IJ)
      IF (FORME(MYROW,MYCOL,NPROW,NPCOL,IPR,JX).EQ.1) THEN
         CALL INDEXCHANGE(NPROW,NPCOL,IPR,JX,II,JJ)
C         CM(IPR,JX)=CM(IPR,JX)+ETK*AX(IJ)+ETS*BX(IJ)+ETC*CX(IJ)
         CM(II,JJ)=CM(II,JJ)+ETK*AX(IJ)+ETS*BX(IJ)+ETC*CX(IJ)
      END IF
17 	CONTINUE
      GO TO 23
...
C     TRANSPOSED FILL
      DO 19 IJ=1,JSNO
      JX=JCO(IJ)
C      WRITE(*,*) '****DANS LE LOOP*****',IJ,JSNO
      IF (FORME(MYROW,MYCOL,NPROW,NPCOL,JX,IPR).EQ.1) THEN
         CALL INDEXCHANGE(NPROW,NPCOL,JX,IPR,II,JJ)
C         CM(JX,IPR)=CM(JX,IPR)+ETK*AX(IJ)+ETS*BX(IJ)+ETC*CX(IJ)
         CM(II,JJ)=CM(II,JJ)+ETK*AX(IJ)+ETS*BX(IJ)+ETC*CX(IJ)
C         WRITE(*,*)MYROW,MYCOL,II,JJ
      END IF
19	CONTINUE
      GO TO 23
...
1     DB10=-999.99
      ...
      ENTRY DB20(X)
      F=20.
      IF (X.LT.1.D-20) GO TO 2
      DB20=F*LOG10(X)     ! Was DB10 = ... (Seemed to work OK)
      RETURN
2     DB20=-999.99
      RETURN
      END
...
4     FORMAT (35H CP TIME TAKEN FOR FACTORIZATION = ,1P,E12.5)
      ...
      SUBROUTINE FACTR (N,A,IP,NDIM,MA,NA)

      INTEGER    N, NDIM, IP(*)
      COMPLEX*16 A(NDIM,*)
      INTEGER    IAM, NPROCS, ICTXT, MYROW, MYCOL, MA, NA
      INTEGER    NPROW, NPCOL, INFO
      INTEGER    DESC_A(9), M, R, C, eerr,MM
      COMPLEX*16, ALLOCATABLE :: A_local(:,:)
      CHARACTER OUTFILE*80
      COMMON /PARA/ ICTXT,IAM,NPROCS,MYROW,MYCOL,NPROW,NPCOL,R,C,MM,NNN

C      WRITE(*,*)'N, NDIM',N,NDIM

      M=N 
C      ALLOCATE (A_local(MA,NA))
C      A_local(1:MA,1:NA)=A(1:MA,1:NA)

C      DO 333 I = 1,MA
C        DO 333 J = 1,NA
C 333       A_local(I,J)=A(I,J) 

      CALL DESCINIT(DESC_A,M,N,R,C,0,0,ICTXT,MA,INFO)

      CALL PZGETRF(M,N,A,1,1,DESC_A,IP,INFO)

C      DO 3333 I = 1,MA
C        DO 3333 J = 1,NA
C 3333      A(I,J)=A_local(I,J)

C      DEALLOCATE (A_local,STAT=eerr)
C      A(1:MA,1:NA)=A_local(1:MA,1:NA)

      IF (INFO.NE.0) THEN
         WRITE(3,*)'ERROR IF FACTR'
         WRITE(*,*)'ERROR IF FACTR',INFO
         CALL BLACS_GRIDEXIT (ICTXT)
         STOP
      END IF
...
C
      COMPLEX*16 A
      INTEGER AUX
      INTEGER ICTXT,IAM,NPROCS,MYROW,MYCOL,NPROW,NPCOL,R,C,MA,NA,MM
      COMMON /PARA/ ICTXT,IAM,NPROCS,MYROW,MYCOL,NPROW,NPCOL,R,C,MM,NNN
      COMMON /MATPAR/ ICASE,NBLOKS,NPBLK,NLAST,NBLSYM,NPSYM,NLSYM,IMAT,I
     1CASX,NBBX,NPBX,NLBX,NBBL,NPBL,NLBL
...
C      ICASE=2
      ...
      MA=MAX(1,NUMROC(NROW,R,MYROW,0,NPROW))
      NA=MAX(1,NUMROC(NROW,C,MYCOL,0,NPCOL))
      DO 1 KK=1,NOP
      KA=(KK-1)*NP+1
      AUX=NROW
      NROW=MM
1     CALL FACTR (NP,A(KA),IP(KA),NROW,MA,NA)
      NROW=AUX
      RETURN
...
C
c$$$      COMPLEX*16 CM,SSX,ZRATI,ZRATI2,T1,ZARRAY,AR1,AR2,AR3,EPSCF,FRATI
c$$$      COMMON /DATA/ X(MAXSEG),Y(MAXSEG),Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),
c$$$     &ALP(MAXSEG),BET(MAXSEG),WLAM,ICON1(2*MAXSEG),ICON2(2*MAXSEG),
c$$$     &ITAG(2*MAXSEG),ICONX(MAXSEG),LD,N1,N2,N,NP,M1,M2,M,MP,IPSYM
c$$$      COMMON /CMB/ CM(IRESRV)
c$$$      COMMON /ANGL/ SALP(MAXSEG)
c$$$      COMMON /GND/ZRATI,ZRATI2,FRATI,T1,T2,CL,CH,SCRWL,SCRWR,NRADL,
c$$$     &KSYMP,IFAR,IPERF
c$$$      COMMON /GGRID/ AR1(11,10,4),AR2(17,5,4),AR3(9,8,4),EPSCF,DXA(3),DY
c$$$     1A(3),XSA(3),YSA(3),NXA(3),NYA(3)
c$$$      COMMON /MATPAR/ ICASE,NBLOKS,NPBLK,NLAST,NBLSYM,NPSYM,NLSYM,IMAT,I
c$$$     1CASX,NBBX,NPBX,NLBX,NBBL,NPBL,NLBL
c$$$      COMMON /SMAT/ SSX(16,16)
c$$$      COMMON /ZLOAD/ ZARRAY(MAXSEG),NLOAD,NLODF
c$$$      COMMON/SAVE/EPSR,SIG,SCRWLT,SCRWRT,FMHZ,IP(2*MAXSEG),KCOM
c$$$      COMMON/CSAVE/COM(19,5)
c$$$C
c$$$C*** ERROR CORRECTED 11/20/89 *******************************
c$$$      DIMENSION T2X(1),T2Y(1),T2Z(1)
c$$$      EQUIVALENCE (T2X,ICON1),(T2Y,ICON2),(T2Z,ITAG)
c$$$C***
c$$$      DATA IGFL/20/
c$$$      OPEN(UNIT=IGFL,FILE='NGF2D.NEC',FORM='UNFORMATTED',STATUS='OLD')
c$$$      REWIND IGFL
c$$$      READ (IGFL) N1,NP,M1,MP,WLAM,FMHZ,IPSYM,KSYMP,IPERF,NRADL,EPSR,SIG
c$$$     1,SCRWLT,SCRWRT,NLODF,KCOM
c$$$      N=N1
c$$$      M=M1
c$$$      N2=N1+1
c$$$      M2=M1+1
c$$$      IF (N1.EQ.0) GO TO 2
c$$$C     READ SEG. DATA AND CONVERT BACK TO END COORD. IN UNITS OF METERS
c$$$      READ (IGFL) (X(I),I=1,N1),(Y(I),I=1,N1),(Z(I),I=1,N1)
c$$$      READ (IGFL) (SI(I),I=1,N1),(BI(I),I=1,N1),(ALP(I),I=1,N1)
c$$$      READ (IGFL) (BET(I),I=1,N1),(SALP(I),I=1,N1)
c$$$      READ (IGFL) (ICON1(I),I=1,N1),(ICON2(I),I=1,N1)
c$$$      READ (IGFL) (ITAG(I),I=1,N1)
c$$$      IF (NLODF.NE.0) READ (IGFL) (ZARRAY(I),I=1,N1)
c$$$      DO 1 I=1,N1
c$$$      XI=X(I)*WLAM
c$$$      YI=Y(I)*WLAM
c$$$      ZI=Z(I)*WLAM
...
c$$$      SI(I)=XI+ALP(I)*DX
c$$$      ALP(I)=YI+BET(I)*DX
c$$$      BET(I)=ZI+SALP(I)*DX
c$$$      BI(I)=BI(I)*WLAM
c$$$1     CONTINUE
c$$$2     IF (M1.EQ.0) GO TO 4
c$$$      J=LD-M1+1
c$$$C     READ PATCH DATA AND CONVERT TO METERS
c$$$      READ (IGFL) (X(I),I=J,LD),(Y(I),I=J,LD),(Z(I),I=J,LD)
c$$$      READ (IGFL) (SI(I),I=J,LD),(BI(I),I=J,LD),(ALP(I),I=J,LD)
c$$$      READ (IGFL) (BET(I),I=J,LD),(SALP(I),I=J,LD)
c$$$C*** ERROR CORRECTED 11/20/89 *******************************
c$$$      READ (IGFL) (T2X(I),I=J,LD),(T2Y(I),I=J,LD)
c$$$      READ (IGFL) (T2Z(I),I=J,LD)
c$$$C      READ (IGFL) (ICON1(I),I=J,LD),(ICON2(I),I=J,LD)
c$$$C      READ (IGFL) (ITAG(I),I=J,LD)
c$$$C
c$$$      DX=WLAM*WLAM
c$$$      DO 3 I=J,LD
c$$$      X(I)=X(I)*WLAM
...
c$$$      Z(I)=Z(I)*WLAM
c$$$3     BI(I)=BI(I)*DX
c$$$4     READ (IGFL) ICASE,NBLOKS,NPBLK,NLAST,NBLSYM,NPSYM,NLSYM,IMAT
c$$$      IF (IPERF.EQ.2) READ (IGFL) AR1,AR2,AR3,EPSCF,DXA,DYA,XSA,YSA,NXA,
c$$$     1NYA
c$$$      NEQ=N1+2*M1
c$$$      NPEQ=NP+2*MP
c$$$      NOP=NEQ/NPEQ
c$$$      IF (NOP.GT.1) READ (IGFL) ((SSX(I,J),I=1,NOP),J=1,NOP)
c$$$      READ (IGFL) (IP(I),I=1,NEQ),COM
c$$$C     READ MATRIX A AND WRITE TAPE13 FOR OUT OF CORE
c$$$      IF (ICASE.GT.2) GO TO 5
c$$$      IOUT=NEQ*NPEQ
c$$$      READ (IGFL) (CM(I),I=1,IOUT)
c$$$      GO TO 10
c$$$5     REWIND 13
c$$$      IF (ICASE.NE.4) GO TO 7
c$$$      IOUT=NPEQ*NPEQ
c$$$      DO 6 K=1,NOP
c$$$      READ (IGFL) (CM(J),J=1,IOUT)
c$$$6     WRITE (13) (CM(J),J=1,IOUT)
c$$$      GO TO 9
c$$$7     IOUT=NPSYM*NPEQ*2
c$$$      NBL2=2*NBLSYM
c$$$      DO 8 IOP=1,NOP
c$$$      DO 8 I=1,NBL2
c$$$      CALL BLCKIN (CM,IGFL,1,IOUT,1,206)
c$$$8     CALL BLCKOT (CM,13,1,IOUT,1,205)
c$$$9     REWIND 13
c$$$10    REWIND IGFL
c$$$C     WRITE(3,N) G.F. HEADING
c$$$      WRITE(3,16)
c$$$      WRITE(3,14)
c$$$      WRITE(3,14)
c$$$      WRITE(3,17)
c$$$      WRITE(3,18)  N1,M1
c$$$      IF (NOP.GT.1) WRITE(3,19)  NOP
c$$$      WRITE(3,20)  IMAT,ICASE
c$$$      IF (ICASE.LT.3) GO TO 11
c$$$      NBL2=NEQ*NPEQ
c$$$      WRITE(3,21)  NBL2
c$$$11    WRITE(3,22)  FMHZ
c$$$      IF (KSYMP.EQ.2.AND.IPERF.EQ.1) WRITE(3,23)
c$$$      IF (KSYMP.EQ.2.AND.IPERF.EQ.0) WRITE(3,27)
c$$$      IF (KSYMP.EQ.2.AND.IPERF.EQ.2) WRITE(3,28)
c$$$      IF (KSYMP.EQ.2.AND.IPERF.NE.1) WRITE(3,24)  EPSR,SIG
c$$$      WRITE(3,17)
c$$$      DO 12 J=1,KCOM
c$$$12    WRITE(3,15)  (COM(I,J),I=1,19)
c$$$      WRITE(3,17)
c$$$      WRITE(3,14)
c$$$      WRITE(3,14)
c$$$      WRITE(3,16)
c$$$      IF (IPRT.EQ.0) RETURN
c$$$      WRITE(3,25)
c$$$      DO 13 I=1,N1
c$$$13    WRITE(3,26)  I,X(I),Y(I),Z(I),SI(I),ALP(I),BET(I)
      RETURN
C
14    FORMAT (5X,50H**************************************************,
     &34H**********************************)
...
c$$$C***
c$$$      DATA IGFL/20/
c$$$      OPEN(UNIT=IGFL,FILE='NGF2D.NEC',
c$$$     1     FORM='UNFORMATTED',STATUS='UNKNOWN')
c$$$      NEQ=N+2*M
c$$$      NPEQ=NP+2*MP
c$$$      NOP=NEQ/NPEQ
c$$$      WRITE (IGFL) N,NP,M,MP,WLAM,FMHZ,IPSYM,KSYMP,IPERF,NRADL,EPSR,
c$$$     1SIG,SCRWLT,SCRWRT,NLOAD,KCOM
c$$$      IF (N.EQ.0) GO TO 1
c$$$      WRITE (IGFL) (X(I),I=1,N),(Y(I),I=1,N),(Z(I),I=1,N)
c$$$      WRITE (IGFL) (SI(I),I=1,N),(BI(I),I=1,N),(ALP(I),I=1,N)
c$$$      WRITE (IGFL) (BET(I),I=1,N),(SALP(I),I=1,N)
c$$$      WRITE (IGFL) (ICON1(I),I=1,N),(ICON2(I),I=1,N)
c$$$      WRITE (IGFL) (ITAG(I),I=1,N)
c$$$      IF (NLOAD.GT.0) WRITE (IGFL) (ZARRAY(I),I=1,N)
c$$$1     IF (M.EQ.0) GO TO 2
c$$$      J=LD-M+1
c$$$      WRITE (IGFL) (X(I),I=J,LD),(Y(I),I=J,LD),(Z(I),I=J,LD)
c$$$      WRITE (IGFL) (SI(I),I=J,LD),(BI(I),I=J,LD),(ALP(I),I=J,LD)
c$$$      WRITE (IGFL) (BET(I),I=J,LD),(SALP(I),I=J,LD)
c$$$C
c$$$C*** ERROR CORRECTED 11/20/89 *******************************
c$$$                                                             
c$$$      WRITE (IGFL) (T2X(I),I=J,LD),(T2Y(I),I=J,LD)
c$$$      WRITE (IGFL) (T2Z(I),I=J,LD)
c$$$C      WRITE (IGFL) (ICON1(I),I=J,LD),(ICON2(I),I=J,LD)
c$$$C      WRITE (IGFL) (ITAG(I),I=J,LD)
c$$$C
c$$$2     WRITE (IGFL) ICASE,NBLOKS,NPBLK,NLAST,NBLSYM,NPSYM,NLSYM,IMAT
c$$$      IF (IPERF.EQ.2) WRITE (IGFL) AR1,AR2,AR3,EPSCF,DXA,DYA,XSA,YSA,NXA
c$$$     1,NYA
c$$$      IF (NOP.GT.1) WRITE (IGFL) ((SSX(I,J),I=1,NOP),J=1,NOP)
c$$$      WRITE (IGFL) (IP(I),I=1,NEQ),COM
c$$$      IF (ICASE.GT.2) GO TO 3
c$$$      IOUT=NEQ*NPEQ
c$$$      WRITE (IGFL) (CM(I),I=1,IOUT)
c$$$      GO TO 12
c$$$3     IF (ICASE.NE.4) GO TO 5
c$$$      REWIND 13
c$$$      I=NPEQ*NPEQ
c$$$      DO 4 K=1,NOP
c$$$      READ (13) (CM(J),J=1,I)
c$$$4     WRITE (IGFL) (CM(J),J=1,I)
c$$$      REWIND 13
c$$$      GO TO 12
c$$$5     REWIND 13
c$$$      REWIND 14
c$$$      IF (ICASE.EQ.5) GO TO 8
c$$$      IOUT=NPBLK*NEQ*2
c$$$      DO 6 I=1,NBLOKS
c$$$      CALL BLCKIN (CM,13,1,IOUT,1,201)
c$$$6     CALL BLCKOT (CM,IGFL,1,IOUT,1,202)
c$$$      DO 7 I=1,NBLOKS
c$$$      CALL BLCKIN (CM,14,1,IOUT,1,203)
c$$$7     CALL BLCKOT (CM,IGFL,1,IOUT,1,204)
c$$$      GO TO 12
c$$$8     IOUT=NPSYM*NPEQ*2
c$$$      DO 11 IOP=1,NOP
c$$$      DO 9 I=1,NBLSYM
c$$$      CALL BLCKIN (CM,13,1,IOUT,1,205)
c$$$9     CALL BLCKOT (CM,IGFL,1,IOUT,1,206)
c$$$      DO 10 I=1,NBLSYM
c$$$      CALL BLCKIN (CM,14,1,IOUT,1,207)
c$$$10    CALL BLCKOT (CM,IGFL,1,IOUT,1,208)
c$$$11    CONTINUE
c$$$      REWIND 13
c$$$      REWIND 14
c$$$12    REWIND IGFL
c$$$      WRITE(3,13)  IGFL,IMAT
      RETURN
C
...
C
     ...
     1,VQD,VQDS,CUX,CM,CMB,CMC,CMD,CMN_l
      INTEGER ICTXT,IAM,NPROCS,MYROW,MYCOL,NPROW,NPCOL,R,C,MA,NA,MM
      INTEGER MMA,NNA
      COMMON /PARA/ ICTXT,IAM,NPROCS,MYROW,MYCOL,NPROW,NPCOL,R,C,MM,NNN
      COMMON /DATA/ X(MAXSEG),Y(MAXSEG),Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),
     ...
     1S(3*MAXSEG), VSRC(30), RHNX(30), CMN_l(30,30)
...
C
      MMA=MAX(1,NUMROC(30,R,MYROW,0,NPROW))
      NNA=MAX(1,NUMROC(30,C,MYCOL,0,NPCOL))

      CALL FACTR (NTEQ,CMN,IPNT,NDIMN,MMA,NNA)
C
...
C
      REAL*8 CPUSECD,Iticks
C-- NOT Customized: return 0
C       CPUSECD = 0 
C-- Customized for Lahey Fortran:
C        Call Timer(Iticks)
C        CPUSECD = Iticks/100D0
        Call CPU_TIME(Iticks)
        CPUSECD = Iticks
      RETURN
      END
...
22    RETURN
      ...
      INTEGER    N, NDIM, IP(*)
      COMPLEX*16 A(NDIM,*), B(*)
C      COMPLEX*16 A(MA,NA), B(*)
      INTEGER    IAM, NPROCS, ICTXT, MYROW, MYCOL, MA, NA
      INTEGER    NPROW, NPCOL, MB, NB, INFO
      INTEGER    DESC_A(9), DESC_B(9),M, R, C,MM
      COMPLEX*16, ALLOCATABLE :: A_local(:,:), B_local(:)
C      COMPLEX*16, ALLOCATABLE :: B_local(:)
      CHARACTER*80 OUTFILE
      COMMON /PARA/ ICTXT,IAM,NPROCS,MYROW,MYCOL,NPROW,NPCOL,R,C,MM,NNN

      M = N

      MA=MAX(1,NUMROC(M,R,MYROW,0,NPROW))
      NA=MAX(1,NUMROC(N,C,MYCOL,0,NPCOL))
      MB=MAX(1,NUMROC(M,R,MYROW,0,NPROW))
      NB=MAX(1,NUMROC(1,C,MYCOL,0,NPCOL))

C      ALLOCATE (A_local(MA,NA),B_local(MB))
      ALLOCATE (B_local(MB))
C      DO 3334 I = 1,MA
C        DO 3334 J = 1,NA
C 3334      A_local(I,J)=A(I,J)
 
      CALL SETARRAY(B,B_local,NPROW,NPCOL,R,C,MYROW,MYCOL,MB, NB, 1 , M)
      
      CALL DESCINIT(DESC_A,M,N,R,C,0,0,ICTXT,MA,INFO)
      CALL DESCINIT(DESC_B,M,1,R,C,0,0,ICTXT,MB,INFO)

      CALL BLACS_BARRIER( ICTXT, 'All' )

      CALL PZGETRS( 'T', N,1,A,1,1,DESC_A,IP,B_local,1,1,DESC_B
     &              ,INFO)

      CALL DETARRAY(B,B_local,NPROW,NPCOL,R,C,MYROW,MYCOL,MB, NB, 1 , M)

      IF (INFO.NE.0) THEN
         WRITE(3,*)'ERROR IF SOLVE'
         WRITE(*,*)'ERROR IF SOLVE'
         CALL BLACS_GRIDEXIT (ICTXT)
         STOP
      END IF
...
3     ZINT=FJ*SQRT(CMOTP/SIGL)*BR1/ROLAM
	...
	SUBROUTINE SETARRAY(AA,A,nprow,npcol,NB,MB,myrow,mycol,lda_x,
     &                       lda_y, N,M ) 
...
!
	...
	INTEGER :: lda_x, lda_y
	COMPLEX*16 :: A(lda_x,lda_y)
	REAL ::ll,mm,Cr,Cc
	INTEGER :: ii,jj,I,J,myrow,mycol,pr,pc,h,g
	INTEGER :: nprow , npcol, NB, MB
	...
	do I=1,M
        ...
        !
            ...
            !
	            ...
	            ll = ABS ((I-1)/(nprow*MB)) 
	            mm = ABS ((J-1)/(npcol*NB))
	            ii = MOD((I-1),MB) + 1 + ABS(INT(ll))*MB
	            jj = MOD((J-1),NB) + 1 + ABS(INT(mm))*NB
...
C	            write (*,*) pr,pc,ii,jj,A(ii,jj)
	...
	SUBROUTINE DETARRAY(AA,A,nprow,npcol,NB,MB,myrow,mycol,
     &                    lda_x, lda_y, N,M ) 
...
c	            jj = MOD(J-1,NB) + 1
                    ...
                    AA(I,J) = A(ii,jj)
C	            write (*,*) pr,pc,ii,jj,A(ii,jj)
                    CALL ZGEBS2D( ICTXT, 'All', ' ', 1, 1, A(ii,jj), 1 )
                 else
  
              CALL ZGEBR2D( ICTXT, 'All', ' ', 1, 1, AA(I,J), 1, pr, pc)
      ...
      INTEGER FUNCTION FORME (MYROW,MYCOL,NPROW,NPCOL,I,J)

      INTEGER :: MYROW,MYCOL,I,J,h,g,pr,pc
      INTEGER :: ICTXT,NPROW,NPCOL,MB,NB,IAM,NPROCS
      REAL    :: ll,mm,Cr,Cc
      INTEGER, PARAMETER :: RSRC=0, CSRC=0
      ...
      SUBROUTINE INDEXCHANGE (NPROW,NPCOL,I,J,ii,jj)

      INTEGER :: MYROW,MYCOL,I,J,h,g,pr,pc,ii,jj
      INTEGER :: ICTXT,NPROW,NPCOL,MB,NB,IAM,NPROCS
      REAL    :: ll,mm,Cr,Cc
...
c-- VARIABLES:
      ...
      integer  m, madd, msub, radd, rsub
      real logp

c-- String for SCCS, do not change!
      character*80 sccsID



c-- Next line is for SCCS, do not change!
      sccsID='%Z%%M% %I% %G% TDB UU'

c-----------------------------------------------------------------------

      m = INT( w + 0.5)
      logp = LOG(FLOAT(p))/LOG(2.0)

c-- CASE 1. M IS A FACTOR IN P
...
c-- CASE 3. P = 2**LOGP, WHERE LOGP IS AN INTEGER
      elseif ( ABS( logp - INT( logp + 0.5)) .le. 1E-6) then
         Split2D = 2**( INT( LOG(w)/LOG(2.0) + 0.5))
...
c--    REPEAT ...
 10      madd = madd + 1
         ...
         rsub = MOD(p,msub)
c--    UNTIL ...
         if ( rsub .ne. 0 .and. radd .ne. 0) goto 10

         if ( radd .eq. 0 .and. rsub .eq. 0) then
            if ( w .gt. m) then
               Split2D = madd
            else
               Split2D = msub
            endif
         elseif ( radd .eq. 0) then
            Split2D = madd
         else
            Split2D = msub
         endif
